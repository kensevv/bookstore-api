...
import com.lvlup.backend.dto.OrderItemResponse
...
import com.lvlup.backend.exception.OrderNotFoundException
...
class OrderService(
    ...
    @Transactional(readOnly = true)
    fun getOrdersPaginatedByUserEmail(userEmail: String, page: Int, size: Int, status: OrderStatus?): PaginatedDataResponse<OrderResponse> {
    ...
    private fun mapToOrderResponse(order: Order): OrderResponse {
        val user = userRepository.findUserByEmail(order.userEmail)!!
